...
(function (root, factory) {
  ...
  var options = {
    ...
    //是否只展示最后一组或者一条记录
    log: null,
    //配置log的展示选项，例如 {color : 'blue', background : #000, fontSize : '20px'}
    warn: '',
    error: ''
  };
  window.LogPag = {};
  ...
  function logEndGroup(callback) {
    var that = window; //log方法调用

    console.log = function (any) {
      var mark = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : new Date().toLocaleTimeString();

      //group状态开启
      if (that.group_start) {
        if (options['log'] && typeof any === 'string' && typeof mark === 'string') {
          var terOut = addStyleToType(any, mark, options['log']);
      ...
      if (console.clear) {
        ...
        if (options['log']) {
          var _terOut = addStyleToType(any, mark, options['log']);

          console.info(_terOut[0], _terOut[1]);
        } else {
          console.info(any, mark);
        }
      } else {
  ...
  function addStyleToType(any, mark, options) {
    ...
    for (var pro in options) {
      ...
      var _match = pro.match(/[A-Z]/) ? pro.match(/[A-Z]/)[0] : {};

      if (_match.length) {
        pro = pro.replace(_match, "-".concat(_match.toLowerCase()));
      }

      _optionStr += "".concat(pro, " : ").concat(options[_pro], ";");
    }

    return ["%c ".concat(any, " ").concat(mark || ''), "".concat(_optionStr)];
  }
